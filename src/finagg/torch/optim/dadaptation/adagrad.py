        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if grad.is_sparse:
                    ...
                    weighted_sk_vals = (
                        weighted_sk.sparse_mask(grad).coalesce()._values()
                    )
                    ...
                    alphak_delta = torch.sparse_coo_tensor(
                        grad.indices(), alphak_delta_vals, grad.shape
                    )
                    ...
                    sksq_weighted_change += (
                        weighted_skp1_vals.sum().item() - weighted_sk_vals.sum().item()
                    )

                    weighted_skp1_delta_vals = weighted_skp1_vals - weighted_sk_vals
                    weighted_skp1_delta = torch.sparse_coo_tensor(
                        grad.indices(), weighted_skp1_delta_vals, grad.shape
                    )
                    ...
                    old_sksq_weighted_param = (
                        (sk * sk).div(torch.sqrt(alphak) + eps).sum().item()
                    )
                    ...
                    sksq_weighted_change += (
                        sksq_weighted_param - old_sksq_weighted_param
                    )
            ...
            for p in group["params"]:
                ...
                if grad.is_sparse:
                    ...
                    loc_delta = torch.sparse_coo_tensor(
                        grad.indices(), loc_delta_vals, grad.shape
                    )
