...
class RefinedNormalizedAnnual:
    ...
    def from_other_refined(
        ...
        *,
        ...
        engine = engine or backend.engine
        company_df = RefinedAnnual.from_refined(
            ticker, start=start, end=end, engine=engine
        ).reset_index(["filed"])
        filed = company_df["filed"]
        industry_df = RefinedIndustryAnnual.from_refined(
        ...
        pct_change_columns = RefinedAnnual.pct_change_target_columns()
    ...
    def get_candidate_ticker_set(
        ...
        """
        return RefinedAnnual.get_ticker_set(lb=lb, engine=engine)
        ...
        *,
        ...
        Args:
            tickers: Set of tickers to install features for. Defaults to all
                ...
                :meth:`RefinedNormalizedAnnual.get_candidate_ticker_set`.
            engine: Feature store database engine. Defaults to the engine
                at :data:`finagg.backend.engine`.
    ...
    def to_refined(
        ...
        *,
        ...
        if set(df.columns) < set(RefinedAnnual.columns):
            raise ValueError(
                f"Dataframe must have columns {RefinedAnnual.columns} but got {df.columns}"
            )
...
class RefinedAnnual(feat.Features):
    ...
    industry = RefinedIndustryAnnual()
    """Annual features aggregated for an entire industry.
    The most popular way for accessing the :class:`RefinedIndustryAnnual`
    feature set.

    :meta hide-value:
    """

    normalized = RefinedNormalizedAnnual()
    """A company's annual features normalized by its industry.
    The most popular way for accessing the :class:`RefinedNormalizedAnnual`
    feature set.
    ...
    def from_api(
        ...
        for concept in cls.concepts:
            ...
            df = get_unique_filings(df, form="10-K", units=units)
    ...
    def install(
        ...
        *,
        ...
        Args:
            tickers: Set of tickers to install features for. Defaults to all
                the candidate tickers from
                :meth:`Refinedannual.get_candidate_ticker_set`.
            engine: Feature store database engine. Defaults to the engine
                at :data:`finagg.backend.engine`.
            recreate_tables: Whether to drop and recreate tables, wiping all
                previously installed data.
...
annual = RefinedAnnual()
"""The most popular way for accessing :class:`RefinedAnnual`.
